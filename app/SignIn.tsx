...
const SignIn = () => {
  ...
  if (!loading && isLogged) return <Redirect href="/" />; // Redirect to home if already logged in

  const handleLogin = async () => {
    const result = await login();

    if (result) {
      refetch({ key: "value" }); // Replace "key" and "value" with actual parameters as needed
    } else {
